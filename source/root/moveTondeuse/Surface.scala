...
object Surface {
  ...
  // On crée ensuite une fonction qui lit et traite les valeurs du fichier Input
  def run(fileName : String): Unit = {

    // On lit le fichier
    val fichierEntree: BufferedSource = io.Source.fromFile(getClass.getResource(s"/$fileName").getFile)

    // On récupére toutes les lignes de ce fichier
    ...
    // On sépare ensuite les deux coordonnées et on les assigne
